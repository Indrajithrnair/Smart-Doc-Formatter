...
from smartdoc_agent.utils.template_utils import apply_business_proposal_template
# We will properly implement these tools later. For now, they are stubs.

# Global reference to current agent instance for placeholder substitution
_current_agent_instance = None

def _substitute_placeholders_in_tool(tool_input_dict: dict) -> dict:
    """Helper function to substitute placeholders in tool inputs"""
    if not _current_agent_instance:
        return tool_input_dict
    
    processed_input = {}
    for key, value in tool_input_dict.items():
        if value == "$FULL_ORIGINAL_ANALYSIS":
            if _current_agent_instance.full_original_analysis_json:
                processed_input[key] = _current_agent_instance.full_original_analysis_json
                print(f"DEBUG: Substituted $FULL_ORIGINAL_ANALYSIS for key '{key}'")
            else:
                raise ValueError(f"Tool tried to use placeholder $FULL_ORIGINAL_ANALYSIS but no original analysis is stored.")
        elif value == "$FULL_MODIFIED_ANALYSIS":
            if _current_agent_instance.full_modified_analysis_json:
                processed_input[key] = _current_agent_instance.full_modified_analysis_json
                print(f"DEBUG: Substituted $FULL_MODIFIED_ANALYSIS for key '{key}'")
            else:
                raise ValueError(f"Tool tried to use placeholder $FULL_MODIFIED_ANALYSIS but no modified analysis is stored.")
        elif value == "$CURRENT_FORMATTING_PLAN":
            if _current_agent_instance.current_formatting_plan_json:
                processed_input[key] = _current_agent_instance.current_formatting_plan_json
                print(f"DEBUG: Substituted $CURRENT_FORMATTING_PLAN for key '{key}'")
            else:
                raise ValueError(f"Tool tried to use placeholder $CURRENT_FORMATTING_PLAN but no plan is stored.")
        else:
...
def analyze_document_structure(tool_input: str | dict) -> str:
    ...
    if isinstance(tool_input, dict):
        ...
        try:
            ...
            if tool_input.strip().startswith('{') and 'doc_path' in tool_input:
                ...
                if isinstance(parsed, dict):
                    doc_path_str = parsed.get("doc_path")
                else:
                    doc_path_str = tool_input.strip().split("\n")[0]
            else:
                # Try to evaluate it as a Python literal (handles dicts-as-strings)
                try:
                    parsed = ast.literal_eval(tool_input.strip())
                    if isinstance(parsed, dict):
                        doc_path_str = parsed.get("doc_path")
                    else:
                        # If it's not a dict, treat the original string as the path
                        doc_path_str = tool_input.strip().split("\n")[0]
                except (ValueError, SyntaxError):
                    # If parsing fails, it's a simple string path
                    doc_path_str = tool_input.strip().split("\n")[0]
    ...
    if not doc_path_str or not isinstance(doc_path_str, str):
    ...
    try:
        ...
        if not os.path.exists(doc_path_str):
            error_msg = f"Document file not found: {doc_path_str}"
            print(f"Tool Error: {error_msg}")
            return json.dumps({"error": error_msg, "file_exists": False, "attempted_path": doc_path_str})
        
        doc = load_document(doc_path_str)
        analysis_data = get_document_analysis(doc)

        analysis_data["document_path"] = doc_path_str
        ...
        result_json = json.dumps(analysis_data, indent=2)
        
        # CRITICAL: Store the result in the agent instance for placeholder substitution
        if _current_agent_instance:
            # Determine if this is original or modified analysis based on the path
            if hasattr(_current_agent_instance, 'current_doc_path') and doc_path_str == _current_agent_instance.current_doc_path:
                _current_agent_instance.full_original_analysis_json = result_json
                print(f"DEBUG: Stored ORIGINAL analysis in agent instance (length: {len(result_json)})")
            elif hasattr(_current_agent_instance, 'current_output_doc_path') and doc_path_str == _current_agent_instance.current_output_doc_path:
                _current_agent_instance.full_modified_analysis_json = result_json
                print(f"DEBUG: Stored MODIFIED analysis in agent instance (length: {len(result_json)})")
            else:
                # Default to original if we can't determine
                _current_agent_instance.full_original_analysis_json = result_json
                print(f"DEBUG: Stored analysis as ORIGINAL (default) in agent instance (length: {len(result_json)})")
        else:
            print("WARNING: No agent instance available to store analysis")
...
if groq_api_key_for_tools and groq_api_key_for_tools not in [
    ...
    try:
        ...
        llm = ChatGroq(groq_api_key=groq_api_key_for_tools, model_name="llama-3.3-70b-versatile", temperature=0) # Llama 3.3 70B versatile model
...
def create_formatting_plan(tool_input: str | dict) -> str:
    ...
    if isinstance(tool_input, str):
        try:
            ...
            cleaned_input = re.sub(r',(\s*[}\]])', r'\1', cleaned_input)
            # Fix any truncated JSON by ensuring it ends properly
            if not cleaned_input.endswith('}') and not cleaned_input.endswith(']'):
                # Try to find the last complete JSON structure
                last_brace = cleaned_input.rfind('}')
                last_bracket = cleaned_input.rfind(']')
                if last_brace > 0 or last_bracket > 0:
                    end_pos = max(last_brace, last_bracket) + 1
                    cleaned_input = cleaned_input[:end_pos]
            
            tool_input = json.loads(cleaned_input)
            ...
            try:
                # Try to extract user_goal manually
                user_goal_match = re.search(r'"user_goal":\s*"([^"]*)"', tool_input)
                if user_goal_match:
                    user_goal = user_goal_match.group(1)
                    # Create a minimal valid input
                    tool_input = {
                        "user_goal": user_goal,
                        "document_analysis_json": {"summary": {"total_elements": 0, "paragraph_count": 0, "heading_count": 0}}
                    }
                    print(f"Recovered user_goal: {user_goal}")
                else:
                    error_msg = f"create_formatting_plan received invalid JSON and could not recover: {str(tool_input)[:200]}..."
                    print(f"Tool Error: {error_msg}")
                    return json.dumps({"error": error_msg})
            except Exception as recovery_error:
                error_msg = f"create_formatting_plan JSON parsing and recovery both failed: {str(recovery_error)}"
                print(f"Tool Error: {error_msg}")
                return json.dumps({"error": error_msg})
    
    if not isinstance(tool_input, dict):
        error_msg = f"create_formatting_plan expects a dictionary or JSON string, got {type(tool_input)}. Value: {str(tool_input)[:200]}"
        print(f"Tool Error: {error_msg}")
        return json.dumps({"error": error_msg})
    ...
    # Add a small sample of elements for context, but limit it to prevent huge prompts
    elements = parsed_analysis_data.get("elements", [])
    if elements:
        # Take first few elements as samples, but limit the text length
        sample_elements = []
        for i, element in enumerate(elements[:5]):  # Only first 5 elements
            sample_element = element.copy()
            # Truncate long text to prevent huge prompts
            if "text" in sample_element and len(sample_element["text"]) > 100:
                sample_element["text"] = sample_element["text"][:100] + "..."
            sample_elements.append(sample_element)
        analysis_summary_for_prompt["sample_elements"] = sample_elements
    
    if not analysis_summary_for_prompt.get("document_path") or not analysis_summary_for_prompt.get("summary"):
        return json.dumps({"error": "Summarized analysis for planning is missing 'document_path' or 'summary'.", "received_analysis_keys": list(parsed_analysis_data.keys())})
...
Available actions:
- "set_heading_font": Changes font properties for headings ONLY.
  - Required: "level" (1, 2, 3, or "all" for all heading levels)
  - Optional: "font_name", "size" (in Pt), "bold", "italic", "color"
  - Optional: "exclude_level" (integer) - When level="all", exclude this specific level
  - Example: {{"action": "set_heading_font", "level": 1, "font_name": "Arial", "size": 18, "bold": true}}
  - Example: {{"action": "set_heading_font", "level": "all", "exclude_level": 1, "font_name": "Calibri", "size": 14}}

- "set_paragraph_font": Changes font properties for all non-heading paragraphs ONLY.
  - Optional: "font_name", "size" (in Pt), "bold", "italic", "color"
  - Example: {{"action": "set_paragraph_font", "font_name": "Times New Roman", "size": 12}}

- "set_paragraph_spacing": Sets spacing for paragraphs.
  - Required: "scope" - Choose based on what needs spacing:
    * "all_paragraphs" or "all_body_paragraphs" = Body paragraphs ONLY (excludes headings)
    * "all_elements" = Both paragraphs AND headings (use for "entire document" requests)
  - Optional: "spacing_before" (in Pt), "spacing_after" (in Pt), "line_spacing" (e.g., 1.0, 1.5, 2.0 for double spacing)
  - Example: {{"action": "set_paragraph_spacing", "scope": "all_body_paragraphs", "spacing_after": 12}}
  - Example: {{"action": "set_paragraph_spacing", "scope": "all_elements", "line_spacing": 2.0}}

- "set_alignment": Changes text alignment.
  - Required: "scope" - Choose based on what needs alignment:
    * "all_paragraphs" or "all_body_paragraphs" = Body paragraphs ONLY (excludes headings)
    * "all_elements" = Both paragraphs AND headings (use for "everything" or "entire document" requests)
  - Required: "alignment" (e.g., "LEFT", "CENTER", "RIGHT", "JUSTIFY")
  - Example: {{"action": "set_alignment", "scope": "all_body_paragraphs", "alignment": "JUSTIFY"}}
  - Example: {{"action": "set_alignment", "scope": "all_elements", "alignment": "LEFT"}}

- "find_and_replace_font": Finds specific text and applies formatting to it.
  - Required: "search_phrase"
  - Optional: "font_name", "size", "bold", "italic", "uppercase", "color"
  - Example: {{"action": "find_and_replace_font", "search_phrase": "CONFIDENTIAL", "font_name": "Arial", "bold": true, "uppercase": true}}

- "fix_font_inconsistencies": Use when document has inconsistent fonts and user asks to "fix" or "unify" formatting.
  - Optional: "target_font_name", "target_font_size"

- "set_document_default_font": Changes the document's base font (use ONLY for "entire document" requests).
  - Use this RARELY - only when user explicitly says "change the entire document font" or "document-wide".
  - Optional: "font_name", "size" (in Pt)

--- CRITICAL RULES ---
1.  **STICK TO THE GOAL**: ONLY generate actions that directly address the user's goal. Do not add extra formatting actions.
2.  **BE SPECIFIC**: 
    - If user asks to format "headings", use ONLY "set_heading_font" action.
    - If user asks to format "paragraphs" or "body text", use ONLY "set_paragraph_font" action.
    - If user asks for both headings AND paragraphs, use separate actions for each.
3.  **NEVER OVER-GENERALIZE**: 
    - DO NOT use "set_document_default_font" unless user explicitly asks to "change the entire document" or "document-wide".
    - DO NOT apply paragraph formatting when only headings are mentioned.
    - DO NOT apply heading formatting when only paragraphs are mentioned.
4.  **MULTI-FONT SCENARIOS**:
    - If user wants different fonts for different heading levels, use multiple "set_heading_font" actions.
    - Use "exclude_level" parameter when applying formatting to "all" headings except one level.

Example scenarios:
Scenario 1: "Make all headings bold and Arial"
[
  {{"action": "set_heading_font", "level": "all", "font_name": "Arial", "bold": true}}
]

Scenario 2: "Change body paragraphs to Times New Roman 12pt"
[
  {{"action": "set_paragraph_font", "font_name": "Times New Roman", "size": 12}}
]

Scenario 3: "Level 1 headings should be Arial 18pt, all other headings and paragraphs should be Calibri 12pt"
[
  {{"action": "set_heading_font", "level": 1, "font_name": "Arial", "size": 18}},
  {{"action": "set_heading_font", "level": "all", "exclude_level": 1, "font_name": "Calibri", "size": 12}},
  {{"action": "set_paragraph_font", "font_name": "Calibri", "size": 12}}
]

Scenario 4: "Make the document professional with Times New Roman throughout and double spacing"
[
  {{"action": "set_heading_font", "level": "all", "font_name": "Times New Roman"}},
  {{"action": "set_paragraph_font", "font_name": "Times New Roman"}},
  {{"action": "set_paragraph_spacing", "scope": "all_elements", "line_spacing": 2.0}}
]

Scenario 5: "Justify all paragraph text" (body paragraphs only, NOT headings)
[
  {{"action": "set_alignment", "scope": "all_paragraphs", "alignment": "JUSTIFY"}}
]

Scenario 6: "Left-align everything in the document" (includes headings)
[
  {{"action": "set_alignment", "scope": "all_elements", "alignment": "LEFT"}}
]

IMPORTANT: Only output the JSON list of actions. Do not include any other text, explanations, or markdown formatting.
The output must be a valid JSON array of objects with NO TRAILING COMMAS.
Ensure your JSON is properly formatted without trailing commas after the last element in arrays or objects.
"""
    ...
    try:
        ...
        try:
            cleaned_plan_str = clean_json_string(generated_plan_str)
            json.loads(cleaned_plan_str) # Try parsing the cleaned version
            
            # CRITICAL: Store the result in the agent instance for placeholder substitution
            if _current_agent_instance:
                _current_agent_instance.current_formatting_plan_json = cleaned_plan_str
                print(f"DEBUG: Stored formatting plan in agent instance (length: {len(cleaned_plan_str)})")
            else:
                print("WARNING: No agent instance available to store formatting plan")
            
            return cleaned_plan_str # Return the cleaned string if it's valid JSON
...
from smartdoc_agent.utils.document_utils import (
    load_document, save_document, get_document_analysis,
    
    # NEW granular handlers that we created
    apply_set_heading_font_action,
    apply_set_paragraph_font_action,
    apply_set_document_default_font_action,
    
    # Existing handlers that we kept
    apply_set_paragraph_spacing_action,
    apply_set_alignment_action,
    apply_fix_font_inconsistencies_action,
    apply_find_and_replace_action,
    apply_find_and_replace_font_action,
    
    # NEW: Content transformation handlers
    apply_markdown_to_headings_action,
    apply_text_to_bullets_action,
    apply_pattern_to_style_action
)
...
def apply_contextual_formatting(tool_input: str | dict) -> str:
    ...
    if isinstance(tool_input, str):
        try:
            ...
            if not cleaned_input.endswith('}') and not cleaned_input.endswith(']'):
                ...
                last_brace = cleaned_input.rfind('}')
                if last_brace > 0:
                    cleaned_input = cleaned_input[:last_brace + 1]
            
            tool_input = json.loads(cleaned_input)
        except json.JSONDecodeError as e:
            error_msg = f"apply_contextual_formatting received invalid JSON string: {str(tool_input)[:200]}... Error: {e}"
            print(f"Tool Error: {error_msg}")
            return json.dumps({"error": error_msg})
    
    if not isinstance(tool_input, dict):
        error_msg = f"apply_contextual_formatting expects a dictionary or JSON string, got {type(tool_input)}. Value: {str(tool_input)[:200]}"
        print(f"Tool Error: {error_msg}")
        return json.dumps({"error": error_msg})

    # CRITICAL: Substitute placeholders before processing
    try:
        tool_input = _substitute_placeholders_in_tool(tool_input)
        print(f"DEBUG: After placeholder substitution, tool_input keys: {list(tool_input.keys())}")
    except Exception as e:
        error_msg = f"Placeholder substitution failed: {str(e)}"
        print(f"Tool Error: {error_msg}")
        return json.dumps({"error": error_msg})
    ...
    try:
        ...
        action_handlers = {
            ...
            "set_heading_font": apply_set_heading_font_action,
            "set_paragraph_font": apply_set_paragraph_font_action,
            "set_document_default_font": apply_set_document_default_font_action,
            ...
            "find_and_replace": apply_find_and_replace_action,
            "find_and_replace_font": apply_find_and_replace_font_action,
            
            # NEW: Content transformation handlers
            "convert_markdown_to_headings": apply_markdown_to_headings_action,
            "convert_text_to_bullets": apply_text_to_bullets_action,
            "convert_pattern_to_style": apply_pattern_to_style_action,
}
        ...
        if os.path.exists(output_doc_path):
            ...
            return json.dumps({
                "status": "error",
                "message": f"Formatting completed but output file was not created at {output_doc_path}.",
                "actions_applied": applied_actions_count,
                "actions_skipped_or_failed": skipped_actions_count,
...
def validate_formatting_result(tool_input: str | dict) -> str:
    ...
    if isinstance(tool_input, str):
        try:
            ...
            error_msg = f"{tool_name} received invalid JSON string: {str(tool_input)[:200]}... Error: {e}"
            print(f"Tool Error: {error_msg}")
            return json.dumps({"error": error_msg})
    ...
    try:
        tool_input = _substitute_placeholders_in_tool(tool_input)
        print(f"DEBUG: After placeholder substitution in {tool_name}, tool_input keys: {list(tool_input.keys())}")
    except Exception as e:
        error_msg = f"Placeholder substitution failed in {tool_name}: {str(e)}"
        print(f"Tool Error: {error_msg}")
        return json.dumps({"error": error_msg})
    ...
    # Include all elements if document is small (<=20 elements), otherwise include first 10
    elements_before = original_analysis.get("elements", [])
    elements_after = modified_analysis.get("elements", [])
    sample_size = len(elements_before) if len(elements_before) <= 20 else 10
    ...
    original_summary = {
        ...
        # Include more elements for better validation
        "sample_elements_before": elements_before[:sample_size]
    }
    modified_summary = {
        ...
        "sample_elements_after": elements_after[:sample_size]
    }
...
"""
    ...
    try:
        ...
        import re
        cleaned_str = validation_report_str.strip()
...
@tool
def apply_business_proposal_template_tool(tool_input: str | dict) -> str:
    """
    Applies a business proposal template to unstructured content.
    Takes raw business content and formats it into a standard business proposal structure.
    
    Input should be a dictionary with:
    - input_doc_path: Path to document with raw business content
    ...
    try:
        ...
        if isinstance(tool_input, str):
            try:
                ...
                return json.dumps({"error": f"Could not parse input: {tool_input}"})
        else:
            parsed_input = tool_input
        
        # Extract required parameters
        input_doc_path = parsed_input.get("input_doc_path")
        output_doc_path = parsed_input.get("output_doc_path")
        template_path = parsed_input.get("template_path")  # Optional
        
        if not input_doc_path or not output_doc_path:
            return json.dumps({
                "error": "Missing required parameters: input_doc_path and output_doc_path are required"
            })
        
        # Apply the template
        result = apply_business_proposal_template(
        ...
        error_msg = f"Error in apply_business_proposal_template_tool: {str(e)}"
        return json.dumps({"error": error_msg})
